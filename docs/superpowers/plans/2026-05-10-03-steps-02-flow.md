## Task 3-2: 步骤 2 重构（流程卡片加载）

**Files:**
- Modify: `workflow_test_desktop/ui/steps/step_flow.py`
- Test: `tests/test_step_flow.py`

- [ ] **Step 1: 重写 step_flow.py**
...
from workflow_test_desktop.config.themes import FONT_FAMILY, SPACE_LG_PX, SPACE_SM_PX
...
class StepFlow(QWidget):
    ...
    def _load_flows(self):
        ...
        self._hint.setVisible(False)
        self._card_grid.setEnabled(False)
    ...
    def _set_loaded(self, flows: list[dict]):
        ...
        self._flows = flows
        self._loading_label.setVisible(False)
        self._card_grid.setEnabled(True)
        self._render_cards("")
        QMetaObject.invokeMethod(self._refresh_btn, "setEnabled", Qt.QueuedConnection, True)
    ...
    def _on_card_clicked(self, flow: dict):
        ...
        for card in self._card_widgets:
            card.set_selected(card._flow == flow)
    ...
    def validate(self) -> tuple[bool, str]:
        if not self._selected_flow:
            self._hint.setText("⚠ 请先选择一个流程")
            self._hint.setVisible(True)
            # 抖动动画
            self._shake_widget(self._hint)
    ...
    def _shake_widget(self, widget):
        ...
        anim.setKeyValueAt(0.25, orig_pos + __import__('PySide6.QtCore', fromlist=['QPoint']).QPoint(10, 0))
        anim.setKeyValueAt(0.5, orig_pos + __import__('PySide6.QtCore', fromlist=['QPoint']).QPoint(-10, 0))
        anim.setKeyValueAt(0.75, orig_pos + __import__('PySide6.QtCore', fromlist=['QPoint']).QPoint(10, 0))
