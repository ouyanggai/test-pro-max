## Task 3-3: 步骤 3 重构（表单填写 + 验证）

**Files:**
- Modify: `workflow_test_desktop/ui/steps/step_form.py`
- Test: `tests/test_step_form.py`

- [ ] **Step 1: 重写 step_form.py**
...
class StepForm(QWidget):
    ...
    def _build_form(self, fields: list):
        ...
        for field in fields:
            ...
            # 设置默认
            if field.get("defaultValue") or field.get("default"):
    ...
    def _make_widget_for_field(self, field: dict) -> QWidget:
        ...
        elif field_type == "number" or field_type == "int" or field_type == "decimal":
            w = QSpinBox()
            w.setFont(FONT_FAMILY)
    ...
    def _set_widget_value(self, widget: QWidget, value):
        if isinstance(widget, QTextEdit):
            ...
            if idx >= 0:
                ...
                idx_text = widget.findText(value)
                if idx_text >= 0:
    ...
    def validate(self) -> tuple[bool, str]:
        ...
        self._error_label.setVisible(False)
        for fid, w in self._field_widgets.items():
            if not isinstance(w, QTextEdit) and not isinstance(w, QSpinBox):
                value = w.text().strip()
